...
| 1.2 | Launch the app | Onboarding modal appears titled "Welcome to Workspace App" | Diagnostics export → file a bug |
| 1.3 | Click **Get started** | Modal dismisses; main 3-pane workspace renders | Capture screenshot |
| 1.4 | Open Settings → Storage section | Data root path shows `~/Library/Application Support/Workspace App/data` (mac) or `%APPDATA%\Workspace App\data` (win) | Path mismatch = file a bug |
